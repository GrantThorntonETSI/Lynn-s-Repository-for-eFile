...
//	});
	...
	$( 'input#iu1a' ).change(function() {
		...
		var m = $( '#gridview table th input:checked' ); //any th input, checked
		var p = $( '#gridview table tr td:nth-child(2) input' ); //GS 2nd checkbox
		var q = $( '#gridview table tr td:nth-child(1) input' ); //GS 1st checkbox
		var n = $( '#gridview table tr td:nth-child(1)' ); //GS item (text) td
		var b = $( '#gridview table tr th:nth-child(1) input:checked' ); //Classnames 1st checkbox, checked
		...
		var cc = $( '#gridview table th:nth-child(1) input:checked' ); //Classnames 1st checkbox, not checked
		var h = $( '#gridview table th input:not(:checked)' ).parent().parent().siblings().children('td').children('input'); //check all GS item checkboxes in their rows
		var z = $(c).parent().parent().siblings('tr').nextUntil('tr:contains(str)').children('td:nth-child(2)').children('input');
		//.not(this).prop('checked', true)
		//input:not(:checked)					
			if((this.checked == true) && $( m == true )) {
				//$('div#yesforeignentity').show( 'fast' );
				...
				//$(p).prop({ //check the iu1a checkbox for all classes
//					checked: true,
//					});
					//console.log(c);
				//$(document).find(c).css('display','none');
				$(c).parent().parent().siblings('tr').nextUntil('tr:contains("Class")').children('td:nth-child(2)').children('input').prop({ //check every GS item 1st checkbox (n) -- whose parents’ parent prev sibling tr has a first-child th child input:checked
					checked: true,
					});
					console.log(z);
			}
			
			else if((this.checked == false) && $( m == false )) {		
